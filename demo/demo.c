...
int main() {
    ...
    if (channels < 1 || channels > MAX_CHANNELS) {
        printf("Invalid channel selection. Defaulting to Stereo.\n");
    ...
    if (rate != 44100 && rate != 48000) {
        printf("Invalid sample rate. Defaulting to 44100 Hz.\n");
    ...
    if (volume < 0.0f || volume > 1.0f) {
        printf("Volume must be between 0.0 and 1.0\n");
        free(buffer);
        return -1;
    }
    ...
    do {
        ...
        if (snd_pcm_writei(playback_handle, buffer, rate * duration) < 0) {
